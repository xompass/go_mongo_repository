...
func lbFilterQuery(filter lbq.Filter, schema *Schema) (MongoFilter, error) {
	where := filter.Where
	...
	if filter.Limit != 0 {
		result.Options.Limit = &filter.Limit
	}
	if filter.Skip != 0 {
		result.Options.Skip = &filter.Skip
	}
...
func getObjectIdArray(val interface{}) ([]primitive.ObjectID, error) {
...
func getObjectIdOrNil(val interface{}) (*primitive.ObjectID, error) {
	if val == nil {
		return nil, nil
	}
...
func getObjectId(val interface{}) (primitive.ObjectID, error) {
	...
	case string:
		oid := v
		value, err := primitive.ObjectIDFromHex(oid)
		return value, err
	case *string:
		oid := v
		value, err := primitive.ObjectIDFromHex(*oid)
...
func getDateArray(val interface{}) ([]time.Time, error) {
...
func getDateOrNil(val interface{}) (*time.Time, error) {
	if val == nil {
		return nil, nil
	}
...
func getDate(val interface{}) (time.Time, error) {
	if val == nil {
