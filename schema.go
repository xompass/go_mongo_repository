...
func (s *Schema) InitFields(val *reflect.Value, jsonParentField string, bsonParentField string) {
	for i := 0; i < val.Type().NumField(); i++ {
		...
		if err := s.InitField(field, jsonParentField, bsonParentField); err != nil {
			fmt.Println(err)
		}
	}
}
...
func (s *Schema) InitRelations(val *reflect.Value) error {
	for i := 0; i < val.Type().NumField(); i++ {
		...
		case reflect.Struct:
			...
			model := val.(IModel)
			_, err := s.parseRelation(fieldStruct, model)
...
func (s *Schema) InitField(fieldStruct reflect.StructField, jsonParentField string, bsonParentField string) error {
	...
	case reflect.Struct:
		...
		default:
			...
			} else if !fieldType.Implements(modelInterface) && !isRelation(fieldStruct) {
...
func isRelation(fieldStruct reflect.StructField) bool {
	if !fieldStruct.IsExported() || fieldStruct.Tag.Get("bson") != "-" {
		return false
	}

	lbTag := strings.TrimSpace(fieldStruct.Tag.Get("lb_rel"))
	return lbTag != "embedded"
}

func (s *Schema) parseRelation(fieldStruct reflect.StructField, model IModel) (bool, error) {
	if !fieldStruct.IsExported() || fieldStruct.Tag.Get("bson") != "-" {
		return false, nil
	}
